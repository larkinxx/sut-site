// ИННфакт: сервер для проверки организаций и ИИ-разбора.
...
// Пересылка к Telegram (на Render, где нет аккаунтов): POST /tg/bot<токен>/<метод> — только методы бота ИННфакт;
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  http.createServer(app).listen(cfg.port, env.HOST || '0.0.0.0', () => {
    console.log(`ИННфакт API: порт ${cfg.port}, DaData ${cfg.dadataToken ? 'есть' : 'НЕТ'}, ИИ ${{ yandex: 'Алиса (' + cfg.yandexModel + ')', gemini: cfg.geminiModel, upstream: 'через ' + cfg.aiUpstream }[aiProvider(cfg)] || 'выключен'}, аккаунты ${db ? env.ACCOUNTS_DB : 'выключены'}, сайты: ${cfg.origins.join(', ')}`);
  });
  if (db) {
