...
const FORBIDDEN = [
  { re: /гарантирован\w*\s+(доход|прибыл|заработ)/i, why: 'обещание гарантированного дохода' },
  { re: /(точно|обязательно)\s+(заработа|разбогате)/i, why: 'обещание заработка' },
  { re: /\b(покупайте|продавайте|вкладывайте\s+в)\b/i, why: 'инвестиционная рекомендация' },
  { re: /без\s+риск(а|ов)/i, why: 'утверждение «без риска»' }
];
