...
<header>
  <div class="bar">
    <a class="logo" href="${url('/')}" aria-label="${esc(site.name)}">${esc(site.name).replace(/^ИНН/, '<b>ИНН</b>')}<i>.</i></a>
...
<section class="calc" id="org"${COMPANY_PAGES ? ' data-pages="1"' : ''} data-api="${esc(process.env.ORG_API_URL || '')}" data-token="${esc(process.env.ORG_API_URL ? '' : (process.env.DADATA_TOKEN || ''))}">
  ...
  <p class="note-sm">Данные берём из открытых реестров через сервис DaData: ИНН отправляется туда для поиска, мы его не храним.${process.env.ORG_API_URL ? ' Разбор готовится автоматически с помощью сервиса Yandex AI Studio (Alice AI) от Яндекса: ему передаются только сведения об организации — из реестров, судов и проверки её сайта — без ФИО, адреса и контактов людей.' : ''}${process.env.ORG_API_URL ? ' Памятка и разбор составлены по общим правилам и не являются' : ' Памятка составлена по общим правилам и не является'} налоговой или юридической консультацией. Сроки и суммы сверяйте на nalog.gov.ru.</p>
</section>
...
<p>Карточку готовит программа с помощью искусственного интеллекта. Обычные новости публикуются автоматически и помечены «подготовлено автоматически»: программа сверяет числа в карточке с текстом первоисточника и проверяет формулировки по правилам (никаких обещаний дохода и призывов «покупайте» или «продавайте»). Редактор такие карточки заранее не читает, поэтому всегда открывайте ссылку на первоисточник. Важные новости (обязательные платежи, сроки, штрафы) и те, в которых программа не уверена, автоматически не публикуются: они ждут проверки человеком, и на такой карточке указано, кто проверил и когда.</p>
...
// ---------- вход, кабинет, политика ----------
{
  ...
  if (POLICY) write('politika/index.html', layout({
    ...
    desc: 'Какие данные обрабатывает сайт ИННфакт, зачем, где они хранятся и как их удалить.',
...
<p class="note-sm">Редакция от ${esc(new Date(FIN.checkedAt + 'T12:00:00+03:00').toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'Europe/Moscow' }))}.</p>
  ...
  if (ACCT) {
    ...
    write('vhod/index.html', layout({
      ...
      desc: 'Вход в кабинет ИННфакт: история проверок, слежение за компаниями, сохранённые расчёты.',
...
// Правовая информация: на каком основании работает сайт, какие законы и открытые данные
{
  ...
  write('pravovaya-informaciya/index.html', layout({
    ...
    desc: 'На каком основании работает сайт ИННфакт, какие законы мы соблюдаем и откуда берём данные об организациях.',
    body: `<h1 class="page">Правовая информация</h1>
<div class="prose">
<p>ИННфакт — информационный сайт. Материалы носят справочный характер и не являются индивидуальной финансовой, налоговой или юридической рекомендацией. Решения по своим делам вы принимаете сами; при необходимости обращайтесь к профильному специалисту.</p>
...
<p>Тексты и оформление сайта ИННфакт — объекты авторского права. При цитировании указывайте источник и ставьте ссылку на страницу. Логотипы и данные ведомств принадлежат их правообладателям.</p>
...
<p>Сайт ведёт редакция ИННфакт. Новости готовятся автоматически по строгим правилам, важные новости перед публикацией проверяет редакция. Подробно об этом — на странице <a href="${url('/kak-my-rabotaem/')}">«Как мы работаем»</a>.</p>
