...
if (!site.siteUrl.includes('example')) {
  ...
  const urls = [
    ['/', fresh], ['/arhiv/', fresh], ['/kalkulyatory/'], ...CALCS.map((c) => [`/kalkulyatory/${c.slug}/`]), ['/organizacii/'], ['/fizlica/'], ['/nalogi/'], ...TAX.map((t) => [`/nalogi/${t.slug}/`]), ['/nalogovye-shemy/'], ['/kak-my-rabotaem/'], ['/o-proekte/'],
