...
const SCHEMES = [
  { t: 'Расходы через «технические» компании',
  ...
  { t: 'Дробление бизнеса',
    ...
    risk: 'Доходы всех частей складывают и пересчитывают налоги по общей системе, с НДС, пенями и штрафом 40%. Если в бизнесе есть реальная деловая причина для раздела (разные направления, города, рынки), это нужно доказать документами.' },
  { t: 'Сотрудники под видом самозанятых или ИП',
  ...
  { t: 'Обналичивание через ИП, самозанятых и подотчёт',
  ...
  { t: 'Формальный переезд ради низкой ставки УСН',
  ...
  { t: '«Займы» учредителю вместо дивидендов',
...
write('nalogovye-shemy/index.html', layout({
  title: 'Опасные налоговые схемы: чего не делать', path: '/nalogovye-shemy/', current: 'org',
...
<p class="lede">Чем заканчиваются популярные способы «сэкономить» на налогах. Здесь нет инструкций: только в чём суть, по каким признакам налоговая это находит и что за это бывает. Законные способы снизить налоги ищите в проверке своей организации по ИНН.</p>
<div class="prose">
${SCHEMES.map((x) => `<section class="scheme"><h2>${esc(x.t)}</h2>
<p><b>В чём суть.</b> ${esc(x.what)}</p>
<p><b>Как находят.</b> ${esc(x.how)}</p>
<p class="risk"><b>Чем заканчивается.</b> ${esc(x.risk)}</p></section>`).join('\n')}
...
if (!site.siteUrl.includes('example')) {
  ...
  const urls = [
    ['/', fresh], ['/arhiv/', fresh], ['/kalkulyatory/'], ...CALCS.map((c) => [`/kalkulyatory/${c.slug}/`]), ['/organizacii/'], ['/fizlica/'], ['/nalogovye-shemy/'], ['/kak-my-rabotaem/'], ['/o-proekte/'],
