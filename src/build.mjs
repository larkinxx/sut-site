...
function feedItem(c) {
  ...
  <div class="more-i">
    ...
    <span class="isrc">${esc(c.source.name)} · ${esc(audNames(c))}</span>
...
<article>
  ...
  <p class="conf"><b>Уровень уверенности: <span class="${conf[0]}">${conf[1]}.</span></b> ${esc(conf[2])}${c.confidenceNote ? ' ' + esc(c.confidenceNote) : ''}<br>Проверил: ${esc(c.review.by)}, ${esc(dateRu(c.review.at))}.</p>
</article>
...
<p>Мы берём новости из официальных источников: Банка России, Минфина, ФНС, Росстата. У каждой карточки есть ссылка на первоисточник, дата и время.</p>
<h2>Кто пишет и проверяет</h2>
<p>Черновик карточки готовит программа с помощью искусственного интеллекта. На сайт он попадает только после проверки редактором: на карточке указано, кто проверил и когда. Программа не может опубликовать новость сама.</p>
