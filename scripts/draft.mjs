...
const SYSTEM = `Ты — редактор русскоязычного сайта ИННфакт. Ты превращаешь новость об экономике (от ведомства или делового СМИ) в короткую карточку: что случилось, кого касается, что можно сделать. Читатель — обычный человек без финансового образования.
...
6. Если источник — СМИ, а не ведомство: пересказывай только подтверждённые факты, без мнений, слухов и прогнозов; новости об отдельных компаниях, котировках акций и курсах валют без последствий для обычных людей пропускай (skip).
7. Отвечай только одним JSON-объектом, без пояснений и без markdown.
...
ФОРМАТ ОТВЕТА
{
  ...
  "calc": "mortgage" | "prepay" | "deposit" | "depositTax" | "selfemployed" | null,
  "critical": true|false,
...
Пояснения: critical=true только если новость может напрямую и скоро изменить платежи, налоги, обязательные сроки или права людей. calc "mortgage" — если речь о ставках по кредитам, "prepay" — о досрочном погашении или рефинансировании, "deposit" — о ставках по вкладам и инфляции, "depositTax" — о налоге на проценты по вкладам, "selfemployed" — о самозанятых, НПД, УСН или взносах ИП, иначе null. В audiences включай только те группы, которых новость реально касается (обычно 1–3).
...
function autoVerdict(card, raw) {
  ...
  if (raw.official === false && card.confidence !== 'high') return 'источник — СМИ, уверенность не высокая';
